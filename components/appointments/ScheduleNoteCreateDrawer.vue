  ...
  <div v-if="open && localDraft" class="fixed inset-0 z-50">
    ...
    <aside class="absolute right-0 top-0 h-full w-full max-w-xl overflow-y-auto bg-white p-4 shadow-xl">
      <div class="mb-4 flex items-start justify-between">
        <div>
          <h2 class="text-lg font-semibold text-slate-900">New Schedule Note</h2>
          <p class="text-xs text-slate-500">Block time for shop, bay, or technician constraints.</p>
        </div>
      ...
      <div class="space-y-3">
        ...
        <label class="block text-sm">
          ...
          <select v-model="localDraft.scheduleBlockerType" class="h-9 w-full rounded-md border border-slate-200 px-2">
            <option v-for="option in scheduleBlockerOptions" :key="option" :value="option">
        ...
        <label class="block text-sm">
          ...
          <select v-model="localDraft.bayId" class="h-9 w-full rounded-md border border-slate-200 px-2">
            <option v-for="bay in bays" :key="bay.bayId" :value="bay.bayId">{{ bay.bayName }}</option>
          </select>
        </label>
...
import type { BayConfig, ScheduleNoteBlockerType } from '@/types/appointment'
import { getScheduleBlockerTypeLabel } from '@/api/appointments'
import { normalizeDuration, toDurationParts, toTotalMinutes } from '@/lib/appointments/duration'

const props = defineProps<{
  open: boolean
  bays: BayConfig[]
  scheduleBlockerOptions: ScheduleNoteBlockerType[]
  draft: {
    customerName: string
...
watch(
  () => props.scheduleBlockerOptions,
  (options) => {
    if (!options.length) return
    if (!options.includes(localDraft.value.scheduleBlockerType as ScheduleNoteBlockerType)) {
      localDraft.value.scheduleBlockerType = options[0]
    }
  },
  { immediate: true }
)
...
const save = () => {
  if (!localDraft.value.scheduleBlockerType) return
  emit('save', {
    ...localDraft.value,
    scheduleBlockerType: localDraft.value.scheduleBlockerType,
